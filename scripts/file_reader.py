...
class FileReader:
    ...
    def check_file_access(self, fp):
        ...
        for a in self.allowed_paths:
            if ap.startswith(os.path.abspath(a)):
                return True
    ...
    def read_file(self, file_path):
        ...
        try:
            ...
            if ft == "pdf":
                self._read_pdf(file_path, result)
            elif ft == "word":
                self._read_docx(file_path, result)
            elif ft == "excel":
                self._read_xlsx(file_path, result)
            elif ft == "csv":
                self._read_csv(file_path, result)
            elif ft == "text":
                self._read_text(file_path, result)
            elif ft == "image":
                self._read_image(file_path, result)
            elif ft in ("excel_legacy",):
                result["error"] = "请转换为.xlsx后重试"
                return result
    ...
    def _analyze_chinese_doc(self, result):
        ...
        if (
            not content
            or len(content) < 200
            or result.get("file_type") in ("image", "excel_legacy")
        ):
            return
        try:
            ...
            if meta.get("is_govt_doc"):
                ...
                result["metadata"]["hierarchy"] = ChineseDocAnalyzer.detect_hierarchy(
                    content
                )
                result["metadata"]["doc_type"] = ChineseDocAnalyzer.classify_doc_type(
                    content
                )
                result["metadata"]["key_sections"] = (
                    ChineseDocAnalyzer.extract_key_sections(content)
                )
                score, reasons = ChineseDocAnalyzer.score_document_quality(
                    content, meta
                )
    ...
    def _read_docx(self, fp, r):
        ...
        r["pages"] = max(1, len(doc.paragraphs) // 30)
        r["read_method"] = "python-docx"
    ...
    def _read_xlsx(self, fp, r):
        ...
        r["pages"] = len(wb.sheetnames)
        r["read_method"] = "openpyxl"
    ...
    def _fmt_table(self, rows):
        ...
        return "\n".join(
            " | ".join(str(c).strip() for c in (list(r) + [""] * (mc - len(r))))
            for r in rows
        )
