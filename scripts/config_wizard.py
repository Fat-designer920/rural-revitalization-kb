...
class ConfigWizard:
    ...
    def create_widgets(self):
        ...
        tk.Label(tf, text="配置向导  v1.0.0", font=("Microsoft YaHei",10), fg="#B0D4F1", bg="#2E75B6").pack()
        ...
        tk.Entry(mf, textvariable=self.api_key_var, font=("Consolas",11), width=55).pack(fill="x", pady=(3,12))
        ...
        pf = tk.Frame(mf); pf.pack(fill="x", pady=(3,12))
        ...
        cf = tk.Frame(mf); cf.pack(fill="x", pady=(3,12))
        ...
        tk.Label(mf, text="4. Notion Token (选填，v1.2.0启用)", font=("Microsoft YaHei",11,"bold"), anchor="w", fg="#999").pack(fill="x", pady=(0,3))
        self.notion_var = tk.StringVar(value=self.existing.get("notion_token_display", ""))
        tk.Entry(mf, textvariable=self.notion_var, font=("Consolas",11), width=55, fg="#999").pack(fill="x", pady=(3,12))

        bf = tk.Frame(mf); bf.pack(fill="x", pady=(10,0))
        tk.Button(bf, text="测试API连接", command=self.test_api, font=("Microsoft YaHei",10), width=14).pack(side="left")
    ...
    def test_api(self):
        ...
        if not key or "****" in key:
            if self.existing.get("deepseek_api_key_encrypted"):
                try: key = decrypt_value(self.existing["deepseek_api_key_encrypted"])
                except: messagebox.showwarning("提示","请先输入API Key"); return
            else: messagebox.showwarning("提示","请先输入API Key"); return
        try:
            import requests
    ...
    def save(self):
        ...
        if not key and not self.existing.get("deepseek_api_key_encrypted"):
            messagebox.showwarning("提示","请输入API Key"); return
