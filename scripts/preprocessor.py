...
class Preprocessor:
    ...
    def preprocess_file(self, fi, doc_origin="external", force_reprocess=False):
        ...
        try:
            ...
            doc_region = None
            try:
                quality_score, q_reasons = ChineseDocAnalyzer.score_document_quality(
                    content
                )
                ...
                if doc_hierarchy:
                    print(
                        f"     层级节点: {len(doc_hierarchy)}个"
                        f"({'/'.join(h['level'] for h in doc_hierarchy[:8])}"
                ...
                result["doc_structure"] = {
                    ...
                    "key_sections": list(
                        ChineseDocAnalyzer.extract_key_sections(content).keys()
                    ),
                }
                    ...
                    if e_status == "completed":
                        if force_reprocess:
                            ...
                            try:
                                ...
                                if ev_del:
                                    print(
                                        f"     已级联清理 operation_events {ev_del} 条"
                                    )
                            ...
                            print(
                                f"     [跳过] 文件已存在(#{e_id} {ename}, 状态:{e_status})"
                            )
            ...
            up = FILE_RENAME_PROMPT["user_prompt_template"].format(
                original_filename=fi["name"],
                file_type=fi["type"],
                content_preview=content[:3000],
            )
            ai = self.client.chat_with_json(
                FILE_RENAME_PROMPT["system_prompt"],
                up,
                temperature=0.2,
                call_type="preprocess",
            )
            parsed = ai.get("parsed_json") or {
                "renamed_filename": Path(fi["name"]).stem,
                "content_type": "policy",
                "domain_tags": [],
                "region_tag": "",
                "policy_level": "",
            ...
            renamed = self._sanitize(
                parsed.get("renamed_filename", Path(fi["name"]).stem)
            )
            fid = self.db.add_source_file(
                fi["name"],
                fi["path"],
                fi["type"],
                int(fi["size_mb"] * 1024 * 1024),
                rr.get("file_hash"),
                doc_origin=doc_origin,
            )
            self.db.update_source_file(
                fid,
                renamed_filename=renamed + ext,
                domain_tags=json.dumps(
                    parsed.get("domain_tags", []), ensure_ascii=False
                ),
                region_tag=parsed.get("region_tag", ""),
                policy_level=parsed.get("policy_level", ""),
            ...
            cnt = 1
            while dest.exists():
                dest = self.processing / f"{renamed}_{cnt}{ext}"
                cnt += 1
            ...
            with open(str(md_path), "w", encoding="utf-8") as mf:
                ...
                if result.get("quality_score"):
                    mf.write(
                        f"> 质量评分: {result['quality_score']}/100 | 类型: {ds.get('doc_type', '未知')}\n"
                    )
            ...
            result.update(
                {
                    ...
                    "file_id": fid,
                    "renamed": dest.name,
                    "content_type": parsed.get("content_type", "policy"),
                }
            )
    ...
    def run(self, doc_origin="external", force_reprocess=False):
        ...
        print(f"\n  发现 {len(files)} 个文件:")
        for f in files:
            print(f"    {f['name']}  [{f['type']}]  {f['size_mb']}MB")
        print(f"\n{'-'*50}\n开始预处理...")
        results, ok, fail = [], 0, 0
        for i, fi in enumerate(files, 1):
            print(f"\n[{i}/{len(files)}]", end="")
            r = self.preprocess_file(
                fi, doc_origin=doc_origin, force_reprocess=force_reprocess
            )
            results.append(r)
            if r["success"]:
                ok += 1
