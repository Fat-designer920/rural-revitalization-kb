...
class PolicyValidator:
    ...
    def _scan_policy_references(self, kps, kps_info):
        ...
        up = prompt["user_prompt_template"].format(
            kp_count=len(kp_batch), knowledge_points_json=kp_json
        )
        ...
        try:
            ...
            uncertain = sum(
                1 for r in results if r.get("involves_policy") == "uncertain"
            )
            print(
                f"     涉及政策{has_policy}条 | 不涉及{no_policy}条 | 待确认{uncertain}条"
            )
        ...
        try:
            ...
            if lock_draft:
                ...
                if kp_detail and kp_detail.get("content_readiness") != "draft":
                    ...
                    print(
                        f"     [锁定] KP#{kp_id}({title}...) 有未验证政策依赖,就绪度降为草稿级"
                    )
    ...
    def run_standalone(self):
        ...
        for start in range(0, len(kps), batch_size):
            ...
            count = self.validate_batch(
                batch_kps, batch_info, "experience"
            )  # 按非政策类处理
