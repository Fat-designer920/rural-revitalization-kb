...
class QaAssistantEngine:
    ...
    def run_qa(
        ...
        # ---- Stage 2: retrieve ----
        self._emit_progress("retrieve", 1, "检索中(关键词 %d 个)" % len(keywords))
        if self._is_canceled():
        ...
        if not candidates:
            # 0 召回 → 直接走 L3
            return self._handle_zero_recall(
                query, mode, is_test_query, start_ts, friend_tag=friend_tag
            )
        ...
        # 速度日志: 分离检索时间和生成时间
        print(
            f"[QA] 检索{self._retrieval_ms}ms + 生成{self._generation_ms}ms = "
        ...
        retrieved_ids = [
            int(kp.get("kp_id")) for kp in top_n if kp.get("kp_id") is not None
        ]
        ...
        try:
            ...
            self._safe_log_event(
                "qa_save_history_failed", "error", {"err": str(e)[:300]}
            )
        ...
        sev = (
            "info"
            if source == "main"
            else ("warning" if source != "rule_fallback" else "error")
        )
        self._safe_log_event(
            evt,
            sev,
    ...
    def _retrieve_and_score(self, keywords: List[str]) -> List[Dict]:
        ...
        try:
            ...
            self._safe_log_event("qa_retrieval_failed", "error", {"err": str(e)[:300]})
            return []
        if not cands:
            return []
        ...
        for kp in cands:
            score = 0.0
            # premium boost
            if (kp.get("premium_client") or 0) == 1 or (
                kp.get("premium_rfp") or 0
            ) == 1:
    ...
    def _rerank_with_v3(self, query: str, candidates: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            self._safe_log_event("qa_rerank_failed", "warning", {"err": str(e)[:300]})
            return candidates[:QA_TOP_N_AFTER_RANK]
    ...
    def _generate_with_fallback_chain(
        ...
        # 主链
        try:
            ans = self._generate_4_panels(query, top_n, model=primary_model)
            if ans:
            ...
            self._safe_log_event(
                "qa_ai_call_failed",
                "warning",
                {"tier": "main", "model": primary_model, "err": last_err},
            )

        # L1 重试(同模型)
        if self._is_canceled():
            return (
                self._l3_rule_fallback(top_n, query),
                "rule_fallback",
                "canceled before L1",
                None,
            )
        self._emit_progress("generate", 3, "L1 重试中(%s)" % primary_model)
        try:
            ans = self._generate_4_panels(query, top_n, model=primary_model)
            if ans:
            ...
            self._safe_log_event(
                "qa_ai_call_failed",
                "warning",
                {"tier": "l1", "model": primary_model, "err": last_err},
            )
        ...
        self._emit_progress("generate", 3, "L2 跨模型兜底(%s)" % l2_fallback_model)
        try:
            ans = self._generate_4_panels(query, top_n, model=l2_fallback_model)
            if ans:
            ...
            self._safe_log_event(
                "qa_ai_call_failed",
                "error",
                {"tier": "l2", "model": l2_fallback_model, "err": last_err},
            )

        # L3 规则兜底
        return self._l3_rule_fallback(top_n, query), "rule_fallback", last_err, None

    def _generate_4_panels(
        self, query: str, retrieved_kps: List[Dict], model: str = "deepseek-chat"
    ) -> Optional[Dict]:
        """V3/R1 调用生成 4 板块.
        ...
        parsed, _, err = self._do_call_json(
            ...
            model=model,
            timeout=timeout,
            call_type="qa_answer_gen",
        )
        if parsed is None:
            # 让上层降级
            self._safe_log_event(
                "qa_answer_parse_failed",
                "warning",
        ...
        if not ev:
            ev = [int(kp.get("kp_id")) for kp in retrieved_kps[:3] if kp.get("kp_id")]
    ...
    def _generate_followups(
        self, query: str, retrieved_kps: List[Dict], all_candidates: List[Dict]
    ) -> List[Dict]:
        """板块 3 备用补救调用. 主链生成的 followup_questions 为空时调."""
        used_titles = [
            kp.get("title", "") for kp in retrieved_kps[:5] if kp.get("title")
        ]
    ...
    def _l3_rule_fallback(self, retrieved_kps: List[Dict], query: str) -> Dict:
        """L3 规则兜底:列 Top 3 KP 标题."""
        top3 = retrieved_kps[:3]
        if not top3:
            return {
                "direct_answer": "抱歉,知识库中暂未检索到与本问题相关的内容. "
                "建议老唐先扩充该主题的精品级知识点.",
                "evidence_kp_ids": [],
                "followup_questions": [],
                "coverage_gap": "本次检索 0 条命中, 当前知识库未覆盖此主题. "
        ...
        for kp in top3:
            ...
            src = (
                kp.get("renamed_filename") or kp.get("original_filename") or "未知来源"
            )
        ...
        return {
            "direct_answer": "本次 AI 服务暂时不稳定, 系统列出库内最相关的 %d 条知识点供老唐自行参考:\n%s"
            % (len(top3), "\n".join(bullets)),
            "evidence_kp_ids": ev_ids,
            "followup_questions": [],
            "coverage_gap": "本次因 AI 调用失败走规则兜底, 板块 3 / 板块 4 暂未生成. "
            "建议查看依据后自行判断, 或稍后重试.",
    ...
    def _retrieve_via_npu(self, query: str, top_k: int = 50) -> List[Dict]:
        ...
        try:
            ...
            results = engine.hybrid_search(query, top_k=min(top_k, len(kp_texts)))
            if not results:
                return []
            ...
            rows.sort(key=lambda x: x.get("_composite_score", 0), reverse=True)
            return rows

        except Exception as e:
            self._safe_log_event(
                "qa_npu_retrieval_failed", "warning", {"err": str(e)[:300]}
            )
            return []
    ...
    def _auto_score_answer(
        self, answer: Dict, query: str, history_id: int, source: str
    ) -> None:
        ...
        try:
            ...
            self._safe_log_event(
                "qa_auto_score_failed", "warning", {"err": str(e)[:300]}
            )
    ...
    def _handle_low_confidence(
        ...
        ans = {
            "direct_answer": "抱歉，当前知识库中暂无与您问题高度匹配的内容。"
            "以下列出库内相对最相关的 %d 条知识点供参考，建议老唐优先补充该主题的精品级知识点。"
            % len(policy_items),
            "evidence_kp_ids": ev_ids,
            "followup_questions": [],
            "coverage_gap": "此问题当前知识库无法充分覆盖，已记录并将优先补充。"
            "建议补充该主题相关的政策原文 / 实操案例 / 投标素材。",
            "policy_basis": policy_items,
    ...
    def _do_call_json(
        ...
        if callable(method):
            try:
                ...
                return (
                    None,
                    content,
                    str(result.get("json_parse_error", "JSON parse failed"))[:300],
                )
        ...
        for mn in ("call_chat", "complete", "call", "generate"):
            ...
            for kw_try in (
                ...
                {"system_prompt": sys_p, "user_prompt": usr_p, "model_override": model},
            ):
                if not is_r1:
    ...
    # 进度 / 取消 / 日志
    # ================================================================
    def _emit_progress(self, current_step: str, processed: int, message: str) -> None:
    ...
    def _canceled_result(self, start_ts: float, msg: str) -> Dict:
        latency_ms = int((time.time() - start_ts) * 1000)
        self._safe_log_event(
            "qa_ask_canceled", "warning", {"msg": msg, "latency_ms": latency_ms}
        )
        return {
            "ok": False,
            "history_id": 0,
