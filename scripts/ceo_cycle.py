...
def perceive():
    ...
    try:
        ...
        c.execute(
            "SELECT content_readiness, COUNT(*) FROM knowledge_points WHERE review_status='confirmed' GROUP BY 1"
        )
...
def run_extract():
    try:
        ...
        subprocess.run(
            [sys.executable, "scripts/run_pipeline.py", "--feed-only"],
            capture_output=True,
            timeout=600,
        )
    except Exception as e:
...
def run_relations():
    try:
        ...
        subprocess.run(
            [sys.executable, "scripts/run_pipeline.py", "--relations-only"],
            capture_output=True,
            timeout=300,
        )
    except Exception:
...
def run_auto_confirm():
    try:
        db = sqlite3.connect("data/database/knowledge_base.db")
        c = db.execute(
            "UPDATE knowledge_points SET review_status='confirmed' WHERE review_status='pending' AND content_readiness IN ('quotable','premium')"
        )
        n = c.rowcount
        db.commit()
        db.close()
...
def run_freshness():
    try:
        db = sqlite3.connect("data/database/knowledge_base.db")
        db.execute(
            "UPDATE knowledge_points SET freshness_checked_at=datetime('now') WHERE freshness_checked_at IS NULL"
        )
        db.commit()
        db.close()
    except Exception:
...
def run_safety_scan():
    try:
        ...
        db.close()
        violations = sum(
            1 for r in rows if semantic_check((r[0] or "") + " " + (r[1] or "")[:200])
        )
        if violations > 0:
    ...
    try:
        ...
        for f in os.listdir(crawled):
            ...
            try:
                ...
                pass
        remaining = len(
            [f for f in os.listdir(crawled) if os.path.isfile(os.path.join(crawled, f))]
        )
        if cleaned > 0:
            log(f"[Audit] Cleaned {cleaned} bad files, {remaining} remain")
    except Exception:
        pass
...
def main():
    ...
    state = perceive()
    log(
        f'Baseline: {state["kp"]}KP/{state["rel"]}Rel/{state["crawl"]}Crawl GPU:{state["gpu"]}'
    )
    ...
    while True:
        ...
        try:
            ...
            if cycle % 10 == 0:
                elapsed = cycle * 30 / 60
                log(
                    f'[Health] C{cycle} KP:{state["kp"]} Rel:{state["rel"]} Crawl:{state["crawl"]}/{state.get("crawl_targets",0)} RAM:{state["ram"]}% Up:{elapsed:.0f}m'
                )
