...
_DOC_TYPE_PATTERNS = {
    ...
    "部门规章": re.compile(
        r"(自然资源部|农业农村部|财政部|生态环境部|住建部).*办法|规定"
    ),
...
class ChineseDocAnalyzer:
    ...
    def score_document_quality(
        cls, text: str, metadata: Optional[Dict] = None
    ) -> Tuple[int, List[str]]:
        ...
        if len(hierarchy) >= 3:
            ...
            reasons.append(
                f"层级结构({len(hierarchy)}个节点) +{min(len(hierarchy),8)*2}"
            )
        ...
        if key_sections:
            ...
            reasons.append(
                f"关键章节({list(key_sections.keys())}) +{min(len(key_sections),5)*3}"
            )
        # 日期
