...
def find_all_py():
    ...
    for pattern in ["scripts/*.py", "agents/*.py"]:
        py_files.extend(
            str(p.relative_to(PROJECT_ROOT)) for p in PROJECT_ROOT.glob(pattern)
        )
    return py_files
                ...
                if isinstance(node.type, ast.Tuple):
                    for elt in node.type.elts:
                        if isinstance(elt, ast.Name) and elt.id == "BaseException":
                            issues.append(
                                f"  L{node.lineno}: except BaseException (too broad)"
                            )
        for node in ast.walk(tree):
            if isinstance(node, ast.ExceptHandler):
                if node.type is not None:
                    line = source.splitlines()[node.lineno - 1]
                    if "except:" in line and isinstance(
                        ast.parse(line.strip().lstrip("except:"))
                    ):
                        pass
    except SyntaxError:
        issues.append("  parse error, skipping AST check")
    except Exception:
        pass
    return issues
...
def check_api_key_leaks(filepath):
    ...
    try:
        ...
        patterns = [
            (
                r"print\s*\(.*(?:api[_-]?key|secret|token|password).*\)",
                "print()包含敏感字段",
            ),
            (
                r"logging\..*\(.*(?:api[_-]?key|secret|token|password).*\)",
                "logging包含敏感字段",
            ),
        ]
