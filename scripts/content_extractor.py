...
POLICY_TYPE_PATTERNS = [
    ...
    (
        "部门规章",
        [
            r"(自然资源部|农业农村部|财政部|国家发展改革委|住房和城乡建设部|生态环境部|水利部)\s*令",
            r"部令\s*第\s*\d+号",
        ],
    ),
...
class GovContentExtractor(object):
    ...
    def _extract_metadata(self, text, page_title, url):
            ...
            if date_match:
                try:
                    ...
                    if 2000 <= y <= 2030 and 1 <= m <= 12 and 1 <= d <= 31:
                        metadata["publish_date"] = "{:04d}-{:02d}-{:02d}".format(
                            y, m, d
                        )
                        break
        ...
        for eff_pat in effective_patterns:
            ...
            if eff_match:
                try:
                    y, m, d = (
                        int(eff_match.group(1)),
                        int(eff_match.group(2)),
                        int(eff_match.group(3)),
                    )
                    if 2000 <= y <= 2030 and 1 <= m <= 12 and 1 <= d <= 31:
                        metadata["effective_date"] = "{:04d}-{:02d}-{:02d}".format(
                            y, m, d
                        )
                        break
                except (ValueError, IndexError):
    ...
    def _assess_quality(self, text, char_count, chinese_count, url):
        ...
        if char_count > 0 and chinese_count / char_count < 0.05:
            return (
                "garbled",
                f"中文字符仅{chinese_count}个({chinese_count/char_count*100:.1f}%)<5%",
            )
        ...
        return (
            "good",
            f"正文{char_count}字,中文{chinese_count}字,核心词{rural_count}个,合格",
        )
...
def check_robots_txt(domain):
    ...
    try:
        ...
        resp = requests.get(
            robots_url,
            timeout=10,
            headers={"User-Agent": "RuralRevitalizationKB/2.3.8"},
        )
