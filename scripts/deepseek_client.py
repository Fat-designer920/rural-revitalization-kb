...
class DeepSeekClient:
    ...
    SILICONFLOW_TEXT_MODEL_L2 = os.getenv(
        "SILICONFLOW_TEXT_MODEL_L2", "Pro/deepseek-ai/DeepSeek-V4-Pro"
    )
    ...
    def chat_with_json(
        ...
        if was_truncated:
            print("       [注意] 模型输出被截断(达到模型输出上限),尝试抢救...")
        ...
        if parsed is None and use_json_mode:
            ...
            if content_abnormal:
                print(
                    "       [JSON Mode 降级] content 异常(空/极短),回退普通模式重试..."
                )
                result_retry = self.chat(
                    json_sys,
                    user_prompt,
                    temperature,
                    max_tokens,
                ...
                retry_parsed, retry_error = self._extract_json_robust(
                    retry_content, retry_truncated
                )
    ...
    def chat_with_jsonl(
        ...
        if not parsed_lines and not meta_object and content.strip():
            fallback_json, fb_err = self._extract_json_robust(
                content, was_truncated=was_truncated
            )
            if fallback_json is not None:
                ...
                if isinstance(fallback_json, dict):
                    if "knowledge_points" in fallback_json and isinstance(
                        fallback_json["knowledge_points"], list
                    ):
                ...
                if kp_objects:
                    print(
                        f"     [JSONL 兼容降级] 7 步解析救回 {len(kp_objects)} 条 kp"
                        f"(原本 0 行)"
                    )
    ...
    def chat_via_siliconflow(
        ...
        resp = self._request(
            self.SILICONFLOW_TEXT_ENDPOINT,
            payload,
            use_model=model,
    ...
    def chat_jsonl_via_siliconflow(
        self,
        system_prompt,
        user_prompt,
        model,
        ...
        """
        result = self.chat_via_siliconflow(
            system_prompt,
            user_prompt,
            model,
            ...
            for attempt in range(1, self.max_retries + 1):
                try:
                    ...
                    if r.status_code == 200:
                        ...
                        cost = round(
                            (inp + out) * 2.0 / 1e6, 6
                        )  # DeepSeek视觉约¥2/百万token
                        self.db.log_api_call(
                            call_type, "deepseek-v4-pro-vision", inp, out, cost
                        )
                        return {
                            "content": content,
                            "input_tokens": inp,
    ...
    def _ocr_single_image(self, image_path, call_type="ocr"):
        ...
        if result:
            print(
                f"     [DeepSeek视觉] OCR成功,费用~{result.get('estimated_cost', 0):.4f}元"
            )
            return result

        print("     DeepSeek视觉不可用,回退硅基流动...")
        # 方案B: 硅基流动(兜底)
        sf_key = self._get_siliconflow_api_key()
        sf_base = self.config.get(
            ...
            "messages": [
                ...
                },
                {
                    ...
                    "content": [
                        ...
                        {
                            "type": "text",
                            "text": "请识别图片中的所有文字，保持原始排版。如有表格请保留表格结构。",
                        },
                    ],
                },
            ],
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                if r.status_code == 403:
                    ...
                    err_code = (
                        err_msg.get("code", "") if isinstance(err_msg, dict) else ""
                    )
    ...
    def _ocr_pdf(self, pdf_path, call_type="ocr"):
        ...
        content = "\n\n".join(all_text)
        print(
            f"     OCR完成! 识别{len(all_text)}/{total_pages}页, 费用~{total_cost:.4f}元"
        )
        return {
            "content": content,
