...
from scripts.backup_manager import BackupFailedError, operation_hook
...
def _qc_readiness_check():
    ...
    # ---- [2] db 关键方法 ----
    for m in (
        "get_qc_rerun_candidates",
        "get_qc_rerun_summary",
...
def _premium_readiness_check():
    ...
    # [2] db 关键方法
    for m in (
        "get_premium_judge_candidates",
        "upsert_premium_ai_cache",
        "get_premium_ai_cache_by_kp",
        "get_premium_pool_list",
        "bless_premium",
        "unbless_premium",
        "get_premium_export_data",
    ):
        if not hasattr(db, m):
            errors.append("[2] db 缺方法: " + m)
    ...
    try:
        ...
        for name, pr in (
            ("PREMIUM_JUDGE_CLIENT_PROMPT", _CP),
    ...
    try:
        ...
        for t in ("qa_history", "qa_feedback"):
            if t not in existing:
                errors.append(
                    "[4] 表 " + t + " 不存在(请先运行 首次安装.bat 追齐 schema)"
                )
    except Exception as e:
...
def _ceo_readiness_check():
    ...
    try:
        ...
        c = conn.cursor()
        c.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name IN ('agent_definitions','audit_cycles')"
        )
        existing = {r[0] for r in c.fetchall()}
        conn.close()
        for t in ("agent_definitions", "audit_cycles"):
            if t not in existing:
                errors.append(
                    "[3] 表 " + t + " 不存在(请先运行 首次安装.bat 追齐 schema)"
                )
    except Exception as e:
...
for _p in [
    ...
    PROJECT_ROOT / "web" / "review.html",
    PROJECT_ROOT / "review.html",
]:
    if _p.exists():
        with open(_p, "r", encoding="utf-8") as _f:
            REVIEW_HTML = _f.read()
        print(f"  [OK] review.html: {_p}")
        break
...
QA_PUBLIC_HTML = None
for _p in [
    PROJECT_ROOT / "web" / "templates" / "qa_public.html",
    PROJECT_ROOT / "web" / "qa_public.html",
    PROJECT_ROOT / "qa_public.html",
]:
    if _p.exists():
        with open(_p, "r", encoding="utf-8") as _f:
            QA_PUBLIC_HTML = _f.read()
        print(f"  [OK] qa_public.html: {_p}")
        break
# v2.3.7-part5: 产品着陆页(LANDING_HTML)
LANDING_HTML = None
for _p in [
    PROJECT_ROOT / "web" / "templates" / "landing.html",
    PROJECT_ROOT / "web" / "landing.html",
    PROJECT_ROOT / "landing.html",
]:
    if _p.exists():
        with open(_p, "r", encoding="utf-8") as _f:
            LANDING_HTML = _f.read()
        print(f"  [OK] landing.html: {_p}")
        break

# v2.3.7-part7: QA产品页(QA_PRODUCT_HTML)
QA_PRODUCT_HTML = None
for _p in [
    PROJECT_ROOT / "web" / "templates" / "qa_product.html",
    PROJECT_ROOT / "web" / "qa_product.html",
    PROJECT_ROOT / "qa_product.html",
]:
    if _p.exists():
        with open(_p, "r", encoding="utf-8") as _f:
            QA_PRODUCT_HTML = _f.read()
        print(f"  [OK] qa_product.html: {_p}")
        break
...
def _safe(item):
    ...
    for k, v in item.items():
        r[k] = v.decode("utf-8", errors="replace") if isinstance(v, bytes) else v
    # 解析所有JSON字段
    for f in [
...
def landing():
    """产品落地页 — 客户入口"""
    if LANDING_HTML:
        return Response(LANDING_HTML, mimetype="text/html; charset=utf-8")
...
def qa_public_page():
    ...
    return Response(
        "<h1>AI问答助手 — 即将上线</h1>", mimetype="text/html; charset=utf-8"
    )
...
def qa_product_page():
    """AI问答助手 — 产品页(新版)"""
    if QA_PRODUCT_HTML:
        return Response(QA_PRODUCT_HTML, mimetype="text/html; charset=utf-8")
    return Response(
        "<h1>AI问答助手 — 产品页加载中...</h1>", mimetype="text/html; charset=utf-8"
    )
...
def get_kps():
    try:
        ...
        if sort_by_qa:
            ...
            items.sort(
                key=lambda x: (x.get("qa_score") is None, x.get("qa_score") or 999)
            )
        ...
        return (
            jsonify(
                {"error": str(e), "items": [], "total": 0, "page": 1, "per_page": 20}
            ),
            500,
        )
...
def confirm(kid):
    try:
        ...
        if cat is None:
            kp = db.get_knowledge_point(kid)
            if kp:
                cat = kp.get("suggested_category_id")
        db.confirm_knowledge_point(
            kid, cat, d.get("final_tags"), d.get("reviewer_notes", "")
        )
...
def ignore(kid):
    try:
        ...
        if kp and kp.get("review_status") == "confirmed":
            db.add_edit_history(
                kid,
                {"review_status": {"old": "confirmed", "new": "ignored"}},
                "移除出库: " + (reason or "无原因"),
            )
...
def update(kid):
    try:
        ...
        if u:
            db.update_knowledge_point(kid, **u)
        return jsonify({"success": True})
    except Exception as e:
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500
        ...
        for kid in ids:
            try:
                ...
                if kp and kp["review_status"] == "pending":
                    db.confirm_knowledge_point(kid, kp.get("suggested_category_id"))
                    n += 1
            except Exception as e:
        ...
        for kid in ids:
            try:
                ...
                if kp and kp["review_status"] in ("pending", "confirmed"):
                    ...
                    db.ignore_knowledge_point(kid, "批量忽略")
                    n += 1
            except Exception as e:
                errors.append({"id": kid, "error": str(e)[:200]})
        return jsonify(
...
def exempt_policy(kid):
    ...
    try:
        ...
        d = request.get_json() or {}
        reason = d.get("reason", "")
        db.update_knowledge_point(
            kid,
            policy_validated=3,
            policy_dependencies=json.dumps(
                [{"exempt_reason": reason or "human_exempt"}], ensure_ascii=False
            ),
        )
                        ...
                        members_detail.append(
                            {
                                ...
                                "ai_extracted_content": kp.get("ai_extracted_content")
                                or "",
                            }
                        )
                    else:
                        members_detail.append(
                            {"id": mid, "title": "(已删除)", "content_type": "unknown"}
                        )
...
def resolve_duplicate(gid):
    ...
    try:
        ...
        for mid in will_delete:
            ...
            deleted.append(mid)
        if deleted:
            action_desc = "保留#%s，删除#%s" % (
                ",".join(str(x) for x in keep_ids),
...
def batch_resolve_duplicates():
    ...
    try:
        ...
        action = d.get(
            "action", "ai_suggest"
        )  # ai_suggest=按AI建议保留, dismiss=全部标记非重复
        if not group_ids:
...
def _confirm_relations_in_group(group_type, gid_int, user="system"):
    ...
    if group_type == "cluster":
        ...
        c = conn.cursor()
        c.execute(
            """SELECT relation_id FROM kp_relations
                      WHERE cluster_id=?
                        AND status IN ('pending','pending_human_review')""",
            (gid_int,),
        )
...
def relations_build_consensus(group_id):
    ...
    try:
        ...
        confirmed = _confirm_relations_in_group(
            gtype, gid_int, user="ui:build_consensus"
        )
        ...
        if gtype == "cluster":
            ...
            db.add_cluster_member(
                cid, rel["source_kp_id"], role="branch", sequence_order=0
            )
            db.add_cluster_member(
                cid, rel["target_kp_id"], role="branch", sequence_order=0
            )
            # 关系绑簇
            conn = db.get_connection()
            c = conn.cursor()
            c.execute(
                "UPDATE kp_relations SET cluster_id=? WHERE relation_id=?",
                (cid, gid_int),
            )
            conn.commit()
            conn.close()
        return jsonify(
            {"success": True, "confirmed": confirmed, "action": "build_consensus"}
        )
    except Exception as e:
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500
...
def relations_build_evolution(group_id):
    ...
    try:
        ...
        topic = (d.get("topic") or "").strip()
        confirmed = _confirm_relations_in_group(
            gtype, gid_int, user="ui:build_evolution"
        )
        if gtype == "cluster":
            ...
            db.add_cluster_member(
                cid, rel["target_kp_id"], role="branch", sequence_order=1
            )
            conn = db.get_connection()
            c = conn.cursor()
            c.execute(
            ...
            conn.close()
        return jsonify(
            {"success": True, "confirmed": confirmed, "action": "build_evolution"}
        )
    except Exception as e:
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500
...
def relations_build_refinement(group_id):
    ...
    try:
        ...
        topic = (d.get("topic") or "").strip()
        confirmed = _confirm_relations_in_group(
            gtype, gid_int, user="ui:build_refinement"
        )
        if gtype == "cluster":
            ...
            db.add_cluster_member(
                cid, rel["source_kp_id"], role="core", sequence_order=0
            )
            db.add_cluster_member(
                cid, rel["target_kp_id"], role="derivative", sequence_order=0
            )
            conn = db.get_connection()
            c = conn.cursor()
            c.execute(
            ...
            conn.close()
        return jsonify(
            {"success": True, "confirmed": confirmed, "action": "build_refinement"}
        )
    except Exception as e:
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500
...
def relations_mark_conflict(group_id):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "confirmed": confirmed, "action": "mark_conflict"}
        )
    except Exception as e:
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500
...
def relations_keep_independent(group_id):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "rejected": rejected, "action": "keep_independent"}
        )
    except Exception as e:
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500
...
def relations_manual_classify(group_id):
    ...
    try:
        ...
        if gtype == "cluster":
            ...
            c = conn.cursor()
            c.execute(
                "UPDATE kp_relations SET relation_type=? WHERE cluster_id=?",
                (rt, gid_int),
            )
            conn.commit()
            conn.close()
            ...
            c = conn.cursor()
            c.execute(
                "UPDATE kp_relations SET relation_type=? WHERE relation_id=?",
                (rt, gid_int),
            )
            conn.commit()
            conn.close()
...
def get_history(kid):
    try:
        return jsonify(db.get_edit_history(kid))
    except Exception:
        return jsonify([])


@app.route(
    "/api/knowledge-points/<int:kid>/restore-version/<int:hid>", methods=["POST"]
)
...
def add_cat():
    try:
        ...
        result = db.add_category(
            l1_code, l1_name, l2_name, desc, is_new_level1=is_new_l1
        )
...
def tree():
    try:
        return jsonify(db.get_categories_tree())
    except Exception:
        return jsonify({})
...
def get_suggestions():
    try:
        return jsonify(db.get_pending_suggestions())
    except Exception:
        return jsonify([])
...
def files():
    try:
        ...
        if s:
            c.execute(
                "SELECT * FROM source_files WHERE process_status=? ORDER BY created_at DESC",
                (s,),
            )
        else:
            c.execute("SELECT * FROM source_files ORDER BY created_at DESC")
...
def debug():
    info = {
        "status": "ok",
        "errors": [],
        "html_loaded": REVIEW_HTML is not None,
        "html_len": len(REVIEW_HTML) if REVIEW_HTML else 0,
    }
    try:
        conn = db.get_connection()
        c = conn.cursor()
...
def dashboard():
    ...
    try:
        ...
        c.execute(
            "SELECT content_type, COUNT(*) FROM knowledge_points GROUP BY content_type"
        )
        by_type = {}
        for row in c.fetchall():
        ...
        c.execute(
            "SELECT SUM(estimated_cost) FROM api_call_logs WHERE call_date=?", (today,)
        )
        ...
        c.execute(
            """SELECT call_type, COUNT(*), SUM(estimated_cost) FROM api_call_logs
                     WHERE call_date=? GROUP BY call_type ORDER BY SUM(estimated_cost) DESC""",
            (today,),
        )
        api_detail = []
        for row in c.fetchall():
            api_detail.append(
                {"type": row[0], "count": row[1], "cost": round(row[2] or 0, 4)}
            )
        data["api_today_detail"] = api_detail

        # 近7天趋势
        c.execute(
            """SELECT call_date, SUM(estimated_cost) FROM api_call_logs
        ...
        c.execute(
            """SELECT CAST(qa_score AS INTEGER) as qs, COUNT(*) FROM knowledge_points
        ...
        try:
            c.execute(
                """SELECT qa_source, COUNT(*) FROM knowledge_points
        ...
        try:
            ...
            # 已设保鲜周期的知识点数
            c.execute(
                "SELECT COUNT(*) FROM knowledge_points WHERE freshness_interval_days IS NOT NULL AND freshness_interval_days > 0"
            )
        ...
        try:
            c.execute(
                """SELECT extracted_by_model, COUNT(*) FROM knowledge_points
            ...
            data["model_distribution"] = {
                ...
                "non_main_pct": (
                    round(md_non_main * 100.0 / md_total, 2) if md_total > 0 else 0.0
                ),
            }
        ...
        try:
            ...
            if cfg_p.exists():
                with open(cfg_p, "r", encoding="utf-8") as f:
                    base = Path(
                        json.load(f).get("knowledge_base_path", str(PROJECT_ROOT))
                    )
        except Exception:
            pass
        ...
        try:
            c2 = conn.cursor()
            c2.execute(
                "SELECT COUNT(*) FROM knowledge_points WHERE source_type='experience_note'"
            )
...
def _qc_rerun_core(progress_cb=None):
    ...
    for fid, kps in by_file.items():
        try:
            ...
            filename = (
                (
                    sf.get("renamed_filename")
                    or sf.get("original_filename")
                    or ("file_%d" % fid)
                )
                if sf
                else ("file_%d" % fid)
            ...
            _report(
                {"current_file": processed_file_count, "processed_kps": total_processed}
            )
        except Exception as ex:
            traceback.print_exc()
    ...
    if orphans:
        try:
            ...
            ext._quality_check(
                "experience_notes", "", kps_list, kps_info, source_content=""
            )
    ...
    _report(
        {
            ...
            % (
                ...
                len(errors),
                (
                    readiness_promote.get("promoted_to_quotable", 0)
                    if isinstance(readiness_promote, dict)
                    else 0
                ),
            ),
        }
...
def qc_rerun_api():
    ...
    if not ok:
        ...
        return (
            jsonify(
                {"success": False, "error": "质检补跑环境未就绪", "errors": errors}
            ),
            500,
        )
...
def premium_pool_refresh():
    ...
    if last_done:
        try:
            elapsed = (
                datetime.now() - datetime.strptime(last_done, "%Y-%m-%d %H:%M:%S")
            ).total_seconds()
    ...
    if not ok:
        try:
            ...
            pass
        return (
            jsonify({"success": False, "error": "启动自检失败", "details": errors}),
            400,
        )
    ...
    with _premium_task_lock:
        ...
        for k in _premium_task["progress"]:
            _premium_task["progress"][k] = (
                0 if isinstance(_premium_task["progress"][k], (int, float)) else ""
            )
    ...
    def _worker():
        try:
            ...
            with _premium_task_lock:
                ...
                _premium_task["error"] = None
                _premium_task["last_completed_at"] = datetime.now().strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
        except Exception as e:
            traceback.print_exc()
            ...
            try:
                db.log_operation_event(
                    event_type="premium_refresh_failed",
                    module="api_server",
                    severity="error",
...
def premium_pool_refresh_cancel():
    ...
    return jsonify(
        {"success": True, "message": "已发送取消请求,引擎将在下一次循环退出"}
    )
...
def premium_pool_refresh_progress():
    ...
    if snap["last_completed_at"] and not snap["running"]:
        try:
            elapsed = (
                datetime.now()
                - datetime.strptime(snap["last_completed_at"], "%Y-%m-%d %H:%M:%S")
            ).total_seconds()
            snap["cooldown_remaining_seconds"] = max(
                0, int(PREMIUM_COOLDOWN_SECONDS - elapsed)
            )
        except Exception:
...
def premium_pool_bless():
    ...
    if len(kp_ids) >= 10:
        try:
            ...
            return (
                jsonify({"success": False, "error": "批量封神前置备份失败: " + str(e)}),
                500,
            )
        except Exception as e:
            # 其他备份异常也阻断(保守)
            traceback.print_exc()
            return (
                jsonify({"success": False, "error": "批量封神前置备份异常: " + str(e)}),
                500,
            )
...
def premium_pool_skip():
    ...
    try:
        db.log_operation_event(
            event_type="premium_skipped",
            module="api_server",
            severity="info",
            related_kp_id=int(kp_id) if kp_id else None,
            payload={"view": view},
        )
    except Exception:
        pass
...
def premium_export():
        ...
        try:
            ...
            return (
                jsonify(
                    {
                        "success": False,
                        "error": "scope=by_category 时 category_id 必填且为整数",
                    }
                ),
                400,
            )
    ...
    try:
        ...
        try:
            db.log_operation_event(
                event_type="premium_export_failed",
                module="api_server",
                severity="error",
    ...
    try:
        db.log_operation_event(
            event_type="premium_export_success",
            module="api_server",
            severity="info",
            payload={
                "scope": scope,
        ...
        return jsonify(
            {
                ...
                "errors": err_count,
                "total": total,
                "message": "已补跑 %d 条(三级降级链)，跳过 %d 个文件/分组"
                % (processed, err_count),
...
def tool_api_cost():
    ...
    try:
        ...
        # 今日费用
        c.execute(
            """SELECT SUM(estimated_cost) FROM api_call_logs
                     WHERE call_date=?""",
            (today,),
        )
        ...
        # 最近7天趋势
        c.execute(
            """SELECT call_date as d, SUM(estimated_cost) FROM api_call_logs
                     WHERE call_date >= date('now','localtime','-7 days')
    ...
    def _run():
        try:
            ...
            with _task_lock:
                _task["result"] = {
                    "success": True,
                    "message": "预处理完成: 成功%d 失败%d" % (ok, fail),
                    "ok": ok,
                    "fail": fail,
                    "skip": 0,
                }
...
def task_reextract():
    ...
    def _run():
        try:
            ...
            _task_update_progress(
                {"current_step": "自动备份", "message": "正在备份数据库..."}
            )
            try:
            ...
            for idx, fid in enumerate(file_ids, 1):
                sf = db.get_source_file(fid)
                fn = (
                    (sf.get("renamed_filename") or sf.get("original_filename", ""))
                    if sf
                    else str(fid)
                )
                _task_update_progress(
                ...
                # 重置源文件状态为processing
                db.update_source_file(
                    fid, process_status="processing", process_message="待重提取(v2.1.2)"
                )

            _task_update_progress(
                {
                    "message": "已删除%d条旧知识点，开始重提取..." % total_deleted,
                    "current_step": "启动提取引擎",
                }
            )
    ...
    threading.Thread(target=_run, daemon=True).start()
    return jsonify(
        {"success": True, "message": "重提取任务已启动", "file_count": len(file_ids)}
    )
...
def task_batch_rerun():
    ...
    threading.Thread(target=_run, daemon=True).start()
    return jsonify(
        {"success": True, "message": "批量重跑已启动", "file_count": len(file_ids)}
    )
...
def _merge_ai_content(kp_row, sc):
    ...
    if (isinstance(desc, str) and desc.strip()) or (
...
def _health_readiness_check():
    ...
    try:
        ...
        if sample:
            ...
            if missing_keys:
                errors.append(
                    "[4] get_kp_for_health_scan 返回 dict 缺字段: "
                    + ", ".join(missing_keys)
...
def health_start():
    ...
    def _run():
        try:
            ...
            _task_update_progress(
                {"current_step": "初始化体检引擎", "message": "加载 HealthChecker..."}
            )
    ...
    threading.Thread(target=_run, daemon=True).start()
    return jsonify(
        {"success": True, "message": "知识库体检已启动", "polish_max": polish_max}
    )
...
def health_suggestions_list(rid):
    ...
    try:
        ...
        for it in items:
            try:
                c.execute(
                    "SELECT title, review_status FROM knowledge_points WHERE id=?",
                    (it.get("kp_id"),),
                )
                row = c.fetchone()
                if row:
                    ...
                    it["kp_current_title"] = (
                        oc.get("title", "") if isinstance(oc, dict) else ""
                    )
...
def health_suggestion_adopt(sid):
    ...
    if not kp_row:
        return (
            jsonify({"error": "知识点已不存在（可能被删除）: kp_id=" + str(kp_id)}),
            404,
        )
...
def health_suggestion_drop(sid):
    ...
    if stype != "drop":
        return (
            jsonify(
                {
                    "error": "非 drop 类型建议请走 /adopt 路由（当前 suggestion_type="
                    + stype
                    + ")"
                }
            ),
...
def _e2e_readiness_check():
    ...
    if p is None:
        errors.append(
            "[1] E2E_RESPONSE_JUDGE_PROMPT 未定义或为 None（对话 A 缺陷 1/2）"
        )
    elif not isinstance(p, dict):
        errors.append(
            "[2] E2E_RESPONSE_JUDGE_PROMPT 不是 dict（实际类型: "
            + type(p).__name__
            + "）"
        )
    else:
        ...
        if not sys_p or not isinstance(sys_p, str):
            errors.append(
                "[2] E2E_RESPONSE_JUDGE_PROMPT 缺 system_prompt 或为空（对话 A 缺陷 4：key 错配）"
            )
        if not usr_p or not isinstance(usr_p, str):
            errors.append(
                "[2] E2E_RESPONSE_JUDGE_PROMPT 缺 user_prompt_template 或为空（对话 A 缺陷 4：key 错配）"
            )
    ...
    required_db_methods = [
        ...
        "save_e2e_test_report",
        "get_latest_e2e_test_report",
        "get_e2e_test_report_detail",
        "get_e2e_test_report_list",
        "upsert_e2e_issue",
        "set_e2e_issue_status",
    ]
...
def e2e_start():
    ...
    def _run():
        try:
            ...
            _task_update_progress(
                {"current_step": "初始化端到端测试引擎", "message": "加载 E2ETester..."}
            )
    ...
    threading.Thread(target=_run, daemon=True).start()
    return jsonify(
        {"success": True, "message": "端到端测试已启动", "scan_depth": scan_depth}
    )
...
def qa_ask():
    ...
    with _qa_task_lock:
        if _qa_task["running"]:
            return (
                jsonify(
                    {"success": False, "error": "另一个问答正在进行,请等待或先 /cancel"}
                ),
                409,
            )
        ...
        try:
            ...
            try:
                db.log_operation_event(
                    ...
                    module="api_server",
                    severity="error",
                    payload={"error": str(e)[:500], "query_preview": query[:200]},
                )
            except Exception:
                pass
        finally:
...
def qa_cancel():
    ...
    with _qa_task_lock:
        if not _qa_task["running"]:
            return jsonify({"success": False, "error": "当前无问答任务"}), 409
        _qa_task["cancel_requested"] = True
    return jsonify(
        {"success": True, "message": "已发送取消请求,引擎将在下一个 stage 退出"}
    )
...
def qa_history_list():
    ...
    try:
        ...
        return jsonify({"success": False, "error": "limit/offset 必须为整数"}), 400
...
def qa_history_detail(hid):
    ...
    try:
        ...
        if not target:
            return (
                jsonify({"success": False, "error": "history_id %d 不存在" % hid}),
                404,
            )
...
def qa_feedback():
    ...
    try:
        ...
        return jsonify({"success": False, "error": "qa_history_id 必填且为整数"}), 400
    ...
    if ft == "comment" and not comment:
        return (
            jsonify({"success": False, "error": "comment 类型时 comment 字段必填"}),
            400,
        )
    ...
    try:
        ...
        try:
            db.log_operation_event(
                event_type="qa_feedback_received",
                module="api_server",
                severity="info",
                payload={
                    "qa_history_id": hid,
...
def audit_history():
    ...
    try:
        ...
        c = conn.cursor()
        c.execute(
            "SELECT cycle_id, cycle_label, status, created_at FROM audit_cycles ORDER BY created_at DESC LIMIT 10"
        )
        rows = [dict(r) for r in c.fetchall()]
        conn.close()
...
def main():
    ...
    print("=" * 60)
    print("  稻也 - 管理后台 v2.3.7-part5")
    print("  Tab1 知识审核 | Tab2 系统管理 | Tab3 智能问答(手机可刷)")
    print("=" * 60)
    ...
    try:
        ...
        print(f"  数据库正常, {c.fetchone()[0]} 条知识点")
        conn.close()
    except Exception as e:
        print(f"  [WARN] DB: {e}")
    print("-" * 60)
    ...
    try:
        ...
        print(f"\n  [FATAL] 端口 {port} 被占用: {e}")
        print("  解决方法: 修改 config/settings.json 中 flask_port 为其他值(如5001)")
        input("\n按回车退出...")
