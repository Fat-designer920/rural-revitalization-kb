...
>
> **当前版本**:**v2.3.5-part2-hotfix1**(hotfix:V4-Pro 全链路切换补完 + L1 镜像 max_tokens 升级 + 跨段补漏分批化 + 标签 prompt 体系一致性根治)— v2.3.5-part2 部署后老唐 0430 喂料 1 个文件暴露的 5 类系统性故障一次性根治。**A 类**:V4-Pro 切换补完 7 处(预分析/结构摘要/跨段补漏/质检批量/质检单条/分类建议/关系判别);**B/C 类**:主提取 max_tokens 32K→65K 给 thinking headroom + L1 硅基镜像 max_tokens 8K→32K + 默认 model R1 镜像→V4-Pro 镜像(settings.json 可覆盖);**D 类**:跨段补漏分批化(>30 kp 自动 30/批 + 严重度优先投票)+ 切 V4-Pro;**E 类**:5 个 _EXTRACT_BASE 老 1.X-5.X 编号清单删除 + _normalize_tag 加近义子串兜底;**F/G 类**:AI 说明 80→300 + R1 文案全清。**立规则 9 第 23 次应验同根 + 立规则 53 第 9 次 + 立规则 57 第 4 次 + 立规则升级**(改造主链必须同步扫降级链 max_tokens / model_id / 标记字段三处 + prompt 与校验体系两侧一致性)。历史版本:v2.3.5-part2 V4-Pro 主链 + 跨段补漏 5 轮闭环 / part1.3 Kimi 双层互备(part2 整体废弃)/ part1.2 timeout 1200s / part1.1 改造遗漏修复 / part1 知识关系网络底座 / v2.3.4-hotfix3 提取链 3 BUG 根治 — 主线无 schema 冲突
