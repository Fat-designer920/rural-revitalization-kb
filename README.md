...
>
> **当前版本**:**v2.3.5-part2**(feature + hotfix:V4-Pro 主链 + 跨段补漏闭环 + Kimi 兜底链全删 + F4/F5 修)— 知识工厂"批量跑知识"前的最后一次系统性重构。主链 R1 → V4-Pro thinking(384K max_output 直接根治 30% 截断率,1M context),跨段补漏从"建议审核时关注"升级为"自动 5 轮重提取闭环 + 基本完整即合格",Kimi 兜底链整体废弃(0429 实测 L1.1+L1.2 三段截断 0/3 救回),降级链 5 层 → 3 层。**立规则 16 第 4 次改造 + 立规则 61 第 2 次应用 + 立规则 63 首次正式落地**(信息齐全度自检 INFO-CHECK)。历史版本:v2.3.5-part1.3 Kimi 双层互备(本版整体废弃)/ part1.2 timeout 1200s / part1.1 改造遗漏修复 / part1 知识关系网络底座 / v2.3.4-hotfix3 提取链 3 BUG 根治 — 主线无 schema 冲突
...
| 阶段 | 做什么 | 验证什么 |
|------|--------|---------|
| 当前(v2.3.5-part2 后) | **V4-Pro 主链 + 跨段补漏闭环(5 轮 + 基本完整即合格)+ 截断防御 3 层架构 + Kimi 兜底链全删 + F4/F5 修(关系判别 100% 失败 + 标签 ~80% 误杀根治)** | 单文件耗时(目标 ≤10 分钟,vs 旧版 9913s)/ V4-Pro 截断率(目标 ≤5%,vs 旧版 30%)/ 跨段补漏闭环平均轮数(目标 1-2 轮)/ F4 关系判别成功率(100% 不再 TypeError)/ F5 标签过滤数量(平均 0-1 个/kp)/ 单文件总成本变化 |
