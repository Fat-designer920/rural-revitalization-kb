...
> **知识工厂**：原料 → 加工 → 质检 → 产品 → 卖钱。底座是知识库，上面长出多种产品形态。基于 DeepSeek V4-Pro(深度推理) + V4-Flash(快速提取) 双模型架构，面向四川乡村振兴领域。
>
> **当前版本**:**v2.3.7**(集团化重构: 6部门16Agent + CEO V4-Pro会议决策)。聚焦四川乡村振兴,锚定月入25万。产品体系: AI政策问答助手(第一优先级)→线上录播课→合规自检→政策日报→模板工具包。远期(月入>¥25万): 操盘手训练营/团队知识库/县级定制报告

## 集团架构(v2.3.7)

**稻也** — 10部门~62个AI Agent,锚定"策划+融资",月入20万目标

| 部门 | 部长 | 核心成员 | KPI |
|------|------|---------|-----|
| CEO办公室 | CEO战略家(V4-Pro) | 财务分析师 + Agent进化师 + 任务铁军 + 死锁哨兵 + 成本卫士 + 进化引擎 + 持续学习引擎 | 方向正确、财务可控 |
| 内容生产部 | 喂料调度员 | 政策研究员 + 案例采编员 + 方法论专家 + 规范研究员(3) + 自动分类师 + 内容包装师 + 管道总监 | 月产≥200条高质量KP |
| 客户交付部 | 方案汇编师(V4-Pro) | 客户视角审查员 + 问答顾问 + 反馈分析师 + 销售页面生成师 | 满意度≥85%、续费率≥60% |
| 市场拓展部 | 获客策略师(V4-Pro) | 内容营销员 + 品牌把关人 + 知乎/抖音/小红书运营员 + 商业情报官 | 月新增付费≥100人 |
| 质量保障部 | 事实核查员 | 保鲜监控员 + 内容保鲜官 | 零事实错误、保鲜率≥95% |
| 技术平台部 | 后勤保障部长 | 系统运维员 | 99.9%在线、NPU/GPU充分利用 |
| 研发中心 | 研发总监(CTO) | 前端架构师 + UI设计师 + 后端工程师 + DBA + 测试架构师 + 代码审查员 + DevOps + 安全审计员 | 技术架构对标大厂标准 |
| 商业变现部 | 定价策略师 | 收入优化师 + 产品包装师 + 销售转化师 + 用户反馈分析器 | 月入20万直接责任部门 |
| 档案管理部 | 档案管理员 | 关系仲裁师 | 文件分类+目录治理+去重+归档 |
| 安全合规部 | 安全卫士 | 防幻觉守卫 | 零有害内容+零幻觉,双门禁一票否决 |

**协作协议**: 老板指令 → CEO V4-Pro深度分析 → 质疑/提替代方案 → 达成共识 → 召集Agent开会辩论(七步协议) → CEO裁决 → 执行 → Agent自我进化升级 → 循环迭代
**自主模式**: CEO run()自主循环(感知→策略→执行→学习→停滞检测→报告),管道日循环全自动,CEO只在异常/低质量时介入
...
| AI 引擎 | DeepSeek API（V4-Pro 深度推理提取 + 打磨；V4-Flash 快速提取 + 批量质检；V3 辅助判断兼容） |
...
| API | 用途 | 模型 |
|-----|------|------|
| DeepSeek | 知识提取 / 打磨 / 分类 / 质检 / 预分析 / 政策扫描 / 关系判别 / 体检分析 / 打磨校验 / 问答 / E2E 语义 | deepseek-v4-pro / deepseek-v4-flash |
...
按提示完成 Python 环境检查、依赖安装、双 API Key 配置、数据库初始化（32 张表一次建成）。
...
- **新增 friend_quota_daily 表 + qa_history.friend_tag 字段 + 1 索引**(数据库 24→32 表,索引 31→48)
