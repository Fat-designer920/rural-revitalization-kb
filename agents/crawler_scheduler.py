...
from scripts.content_extractor import (
    EXCLUDE_KEYWORDS,
    GovContentExtractor,
    check_robots_txt,
)
...
class CrawlerScheduler(object):
    ...
    def run(self, max_articles=MAX_ARTICLES_PER_RUN, sources=None):
        ...
        report_path = (
            review_dir / f"crawl_report_{datetime.now().strftime('%Y%m%d_%H%M')}.json"
        )
    ...
    def _fetch_and_extract_article(
        self, link, review_dir, source_domain="", source_name=""
    ):
    ...
    def _doc_number_exists(self, doc_number):
        ...
        try:
            ...
            c = conn.cursor()
            c.execute(
                "SELECT COUNT(*) FROM crawl_history WHERE doc_number=?", (doc_number,)
            )
            count = c.fetchone()[0]
            conn.close()
            return count > 0
    ...
    def _fetch_url(self, url, timeout=30):
        ...
        try:
            ...
            meta_match = re.search(
                rb'charset[="\s]+([a-zA-Z0-9_-]+)', head_sample, re.IGNORECASE
            )
            if meta_match:
                try:
    ...
    def _record_crawl(
        self, url, content_hash, saved_path, doc_number="", source_domain=""
    ):
        """记录爬取历史到DB"""
        if not self.db:
            return
    ...
    def _safe_filename(self, url, page_title=""):
        ...
        domain_short = (
            domain.replace("www.", "").replace(".gov.cn", "").replace(".", "_")[:30]
        )
    ...
    def get_status(self):
        ...
        status = {
            ...
            "listing_sources": len(POLICY_LISTING_SOURCES),
            "last_run": None,
        }
        if self.db:
            try:
