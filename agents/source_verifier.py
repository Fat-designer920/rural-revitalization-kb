    ...
    def seed_whitelist(self):
        ...
        try:
            ...
            for domain, cat, desc in domains:
                c.execute(
                    """INSERT OR IGNORE INTO source_whitelist (domain, category, description)
                             VALUES (?,?,?)""",
                    (domain, cat, desc),
                )
                if c.rowcount > 0:
    ...
    def verify(self, source_url, doc_origin=""):
        ...
        if self.db and domain:
            try:
                ...
                c = conn.cursor()
                c.execute(
                    """SELECT category FROM source_whitelist
