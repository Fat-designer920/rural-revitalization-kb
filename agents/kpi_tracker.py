...
WORLD_CLASS = {
    "factual_error_rate": 0.001, "audit_coverage": 0.10,
    "freshness_rate": 0.95, "csat": 0.92,
    "agent_zombie_ratio": 0.05, "red_flag_rate": 0.05,
}
...
class KPITracker:
    ...
    def __init__(self, db=None):
        if db is None: self._db_path, self._ext_db = DB_PATH, None
        elif hasattr(db, 'get_connection'):
            self._ext_db, self._db_path = db, getattr(db, 'db_path', DB_PATH)
        else: self._db_path, self._ext_db = str(db), None
        self._snapshots = []
    def _conn(self):
        if self._ext_db: return self._ext_db.get_connection()
    ...
    def quality_metrics(self):
        ...
        try:
            ...
            r = c.execute(
                "SELECT AVG(qa_score) FROM knowledge_points WHERE qa_score > 0").fetchone()
            m["avg_qa"] = round(r[0], 2) if r and r[0] else 0
            m["red_flag"] = c.execute(
            ...
            fresh = c.execute(
                "SELECT COUNT(*) FROM knowledge_points WHERE review_status='confirmed'"
                " AND is_outdated=0").fetchone()[0] or 0
            m["freshness_rate"] = round(fresh / m["confirmed"], 4) if m["confirmed"] else 1.0
            m["outdated"] = c.execute(
            ...
            if ar and ar[0] and m["confirmed"]:
                try:
                    ...
                    pass
            cx = c.execute(
                "SELECT COUNT(*) FROM qa_feedback WHERE feedback_type='helpful'").fetchone()[0] or 0
            cn = c.execute(
                "SELECT COUNT(*) FROM qa_feedback WHERE feedback_type='not_helpful'").fetchone()[0] or 0
            m["satisf_n"] = cx + cn
    ...
    def production_metrics(self):
        ...
        try:
            c = conn.cursor()
            m["total"] = c.execute(
                "SELECT COUNT(*) FROM knowledge_points").fetchone()[0] or 0
            m["confirmed"] = c.execute(
                "SELECT COUNT(*) FROM knowledge_points WHERE review_status='confirmed'"
            ).fetchone()[0] or 0
            m["pending"] = c.execute(
            ...
            m["reader_tag_rate"] = round(tagged / m["confirmed"], 4) if m["confirmed"] else 0
            types = {}
    ...
    def agent_metrics(self):
        ...
        try:
            c = conn.cursor()
            m["total"] = c.execute(
                "SELECT COUNT(*) FROM agent_definitions").fetchone()[0] or 0
            m["active"] = c.execute(
            ...
            r = c.execute(
                ...
                " WHERE call_date >= ?", (month_ago,)).fetchone()
    ...
    def infrastructure_metrics(self):
        ...
        m = {"db_mb": 0, "pending_files": 0, "health_score": 0, "err_7d": 0, "warn_7d": 0}
        try:
    ...
    def gap_vs_world_class(self):
        ...
        gaps = {}
        for key, cur in [("factual_error_rate", q.get("red_flag_rate", 0)),
            ...
            status = "OK" if gap_val <= 0 else ("WARN" if gap_val < 0.10 else "CRITICAL")
            gaps[key] = {"current": cur, "target": wc[key], "gap": gap_val, "status": status}
        return gaps
    def report(self):
        """生成KPI报告(Markdown格式,可存入文件或发给CEO)。"""
        snap = self.snapshot()
        q, p, a, inf = snap["quality"], snap["production"], snap["agents"], snap["infrastructure"]
        ...
        for name, g in gaps.items():
            icon = "OK" if g["status"] == "OK" else ("WARN" if g["status"] == "WARN" else "!!")
            lines.append(f"- [{icon}] {name}: {g['current']:.1%} vs {g['target']:.1%}"
                         f" (gap: {g['gap']:+.1%})")
    ...
    def _trends(self, cur, prev):
        out = []
        for sec, label in [("quality","质量"), ("production","生产"), ("agents","Agent")]:
            for k, v in cur.get(sec, {}).items():
                if not isinstance(v, (int, float)): continue
                pv = prev.get(sec, {}).get(k, v)
                if pv != v:
                    d = v - pv
                    out.append(f"- {label}.{k}: {pv} -> {v} ({'+' if d>=0 else ''}{d:+.2f})")
        return "\n".join(out[:20])
