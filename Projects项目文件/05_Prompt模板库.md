...
### 标签策略（TAG_STRATEGY，嵌入5个提取Prompt）
- 合格标签6类：政策名称、地域、业务场景、项目阶段、关键主体、核心方法论
- 禁止标签：具体数值、具体日期、过于宽泛、过于琐碎、重复分类名
- 每个知识点3-6个标签，使用行业通用术语，4-10字为宜
- 标签服务目标：生成知识产品、客户对话助手、AI私域咨询

### 原文摘录要求（EXCERPT_REQUIREMENT，嵌入5个提取Prompt）
- 引用完整段落（50-300字），不要只摘一句话
- 保留上下文，脱离原文也能看懂
- 原文精度：数值名称表述必须一致
- 质量标准：可直接复制到文章中使用（服务于未来辅助写文章需求）
...
| 2026-03-19 | v1.0.1 | 新增TAG_STRATEGY标签策略，嵌入5个提取Prompt |
| 2026-03-19 | v1.0.1 | 新增EXCERPT_REQUIREMENT原文摘录要求，服务写文章需求 |
