...
## v2.2.2 质量管控增强版 (2026-04-16)

### F051 重复检测AI升级
- **修复根因**: DuplicateChecker在工具箱调用时未传入DeepSeekClient, V3精判从未执行, 导致200+组假阳性
- 无client时不再创建重复组(之前会用本地默认判断创建假阳性)
- V3判定为"互补"的知识点不再建组(只有重复/更替/冲突才建组)
- 阈值收紧: 标题0.50->0.65, 关键词0.40->0.50, 综合0.55->0.65
- V3输入增加原文摘录(之前只有200字内容摘要)
- 新增"清理并重扫"功能: 一键清除所有pending假阳性后用V3重新全库扫描

### F052 仪表盘穿透导航
- 仪表盘所有数据行支持点击跳转到知识审核Tab并自动应用对应筛选
- 穿透覆盖: 审核状态/内容类型/就绪度/质检分数/保鲜状态/政策校验/重复待处理
- 新增dashJump()统一跳转函数
- 可点击行有hover高亮效果

### F053 质检分数筛选器
- 侧边栏新增"质检分数"筛选区(5分/4分/3分/2分/1分/未质检)
- 后端db_manager.py新增qa_score_filter参数
- api_server.py透传qa_score参数到列表和全选接口
- 修复source_type_filter缺少experience_note的处理

### F054 质检补跑
- 工具箱新增"质检补跑"按钮
- 对所有qa_score IS NULL的知识点补跑V3质检
- 后端新增/api/tools/qa-backfill接口

### 涉及文件
- scripts/api_server.py (v2.2.2)
- web/templates/review.html (v2.2.2)
- scripts/duplicate_checker.py (v2.2.2)
- scripts/db_manager.py (v2.2.2)
